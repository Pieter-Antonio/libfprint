...
#define FP_COMPONENT "goodixtls53x5"
...
struct _FpiDeviceGoodixTls53x5 {
  FpiDeviceGoodixTls parent;
};

G_DECLARE_FINAL_TYPE(FpiDeviceGoodixTls53x5, fpi_device_goodixtls53x5, FPI,
                     DEVICE_GOODIXTLS53x5, FpiDeviceGoodixTls);

G_DEFINE_TYPE(FpiDeviceGoodixTls53x5, fpi_device_goodixtls53x5,
...
static void check_firmware_version(FpDevice *dev, gchar *firmware,
  ...
  if (strcmp(firmware, GOODIX_53X5_FIRMWARE_VERSION)) {
...
static void check_preset_psk_read(FpDevice *dev, gboolean success,
  ...
  if (length != sizeof(goodix_53x5_psk_0)) {
    g_set_error(&error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA,
  ...
  if (memcmp(psk, goodix_53x5_psk_0, sizeof(goodix_53x5_psk_0))) {
    g_set_error(&error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA,
                "Invalid device PSK: 0x%s", psk_str);
    fpi_ssm_mark_failed(user_data, error);
    return;
...
static void activate_run_state(FpiSsm *ssm, FpDevice *dev) {
  ...
  switch (fpi_ssm_get_cur_state(ssm)) {
    ...
    case ACTIVATE_CHECK_PSK:
      goodix_send_preset_psk_read(dev, 0, 0,
...
static void fpi_device_goodixtls53x5_init(FpiDeviceGoodixTls53x5 *self) {}

static void fpi_device_goodixtls53x5_class_init(
    FpiDeviceGoodixTls53x5Class *class) {
  ...
  gx_class->interface = GOODIX_53X5_INTERFACE;
  gx_class->ep_in = GOODIX_53X5_EP_IN;
  gx_class->ep_out = GOODIX_53X5_EP_OUT;

  dev_class->id = "goodixtls53x5";
  dev_class->full_name = "Goodix TLS Fingerprint Sensor 53x5";
