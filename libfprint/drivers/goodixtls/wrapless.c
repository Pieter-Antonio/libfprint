...
void goodix_send_protocol(FpDevice *dev, guint8 cmd, guint8 *payload,
  ...
  if (timeout_ms) {
    ...
    fp_dbg("send_protocol timeout reached");
  }
  priv->cmd = cmd;
  priv->ack = TRUE;
  priv->reply = reply;
  ...
  fp_dbg("send_protocol starting encode_protocol");
  ...
  if (!goodix_send_pack(dev, GOODIX_FLAGS_MSG_PROTOCOL, data, data_len, g_free,
                        &error)) {
    fp_dbg("send_protocol starting receive_done");
    goodix_receive_done(dev, NULL, 0, error);
    return;
  };
}
...
void goodix_send_nop(FpDevice *dev, GoodixNoneCallback callback,
  ...
  if (callback) {
    ...
    fp_dbg("send_nop starting send_protocol");
    ...
    fp_dbg("send_nop starting receive_done");
